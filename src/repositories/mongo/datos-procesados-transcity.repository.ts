...
import { TranscityInterface } from '../../interface/transcity.interface';
import { CreateIncidenteDTO } from '../../dtos/Mongo/crear-inicidente.dto';

@Injectable()
export class DatosProcesadosTranscityRepository {
  constructor(@InjectModel('DatosIncidente')
  private incidenteModel: Model<TranscityInterface>) {
  }


  async create(CrearIncidenteDTO: any): Promise<any> {
  ...
  async findById(id): Promise<TranscityInterface> {
  ...
  async find(req: string): Promise<any> {
    return await this.incidenteModel.find({ id: { $eq: req } }).exec();
  }
  ...
  async update(id, CrearIncidenteDTO: any): Promise<any> {
    console.log(id, CrearIncidenteDTO.decision_colaI);
